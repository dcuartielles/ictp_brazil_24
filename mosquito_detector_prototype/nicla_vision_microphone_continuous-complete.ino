...
/* Includes ---------------------------------------------------------------- */
#include <mosquiteiro_inferencing.h>
#include <PDM.h>

#define PIN_HUMIDIFIER  3

long timer3 = 0;
int mosquitoDetected = 0;
...
void loop()
{
    ...
    if (++print_results >= (EI_CLASSIFIER_SLICES_PER_MODEL_WINDOW)) {
        ...
        if (result.classification[1].value >= 0.70 && !mosquitoDetected) {
          ei_printf("MOSQUITO DETECTED!!!");
          digitalWrite(PIN_HUMIDIFIER, LOW);
          delay(400);
        ...
        if (millis() - timer3 > 5000 && mosquitoDetected) {
          ei_printf("MOSQUITO TIMEOUT!!!");
          digitalWrite(PIN_HUMIDIFIER, LOW);
          delay(400);
          digitalWrite(PIN_HUMIDIFIER, HIGH);
